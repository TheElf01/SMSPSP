 ...
 /*NOTA*/
  ...
  while (!ExitPSP)
  {
    ...
    while (!ExitPSP)
    {
      ...
      /* Draw current path */
      pspVideoPrint(UiMetric.Font, sx+10, UiMetric.Top, cur_path,
        UiMetric.TitleColor);
        ...
        for (f = 1; f <= n; f++)
        {
          ...
          if (screenshot)
            pspVideoPutImage(screenshot,
                            (UiMetric.Right-8) - screenshot_width - UiMetric.ScrollbarWidth,
                            ((UiMetric.Bottom - UiMetric.Top) / 2 - 
                            screenshot_height / 2) + (UiMetric.Top-20),
                            screenshot_width,
                            screenshot_height);

          /* Selection box */
          int box_top = last_sel_top-((last_sel_top-sel_top)/n)*f;
          pspVideoFillRect(sx, sel_top, sx+(w-178), sel_top+fh,
      ...
      if (screenshot)
        pspVideoPutImage(screenshot,
                         UiMetric.Right+120 - screenshot_width - UiMetric.ScrollbarWidth,
                         ((UiMetric.Bottom - UiMetric.Top) / 2 - 
                         screenshot_height / 2) + UiMetric.Top,
                         screenshot_width/2,
                         screenshot_height/2);

     /* NOTA: Barra de seleccion rom */
      if (sel) pspVideoFillRect(sx, sel_top, sx+(w-178), sel_top+fh,
        UiMetric.SelectedBgColor);
...
void pspUiOpenMenu(PspUiMenu *uimenu, const char *title)
{
  ...
  while (!ExitPSP)
  {
    ...
    /* NOTA: Draw the highlight for selected item */
    if (!option_mode)
      pspVideoFillRect(sx, sel_top, sx+(w-168), sel_top+fh, 
        UiMetric.SelectedBgColor);
