...
#define OPTION_OF 0x0C
#define OPTION_REGION 0x0D
#define OPTION_BORDER 0x0E
#define OPTION_POSX 0x0F
...
static const char *QuickloadFilter[] = { "ZIP", '\0' },
...
static void LoadOptions_game();
static int  SaveOptions();
static int CreateOptions();
static void loadborder();
...
PL_MENU_OPTIONS_BEGIN(SoundEngineOptions)
  PL_MENU_OPTION("Disabled",      SND_NULL)
  PL_MENU_OPTION("Faster",        SND_YM2413)
  PL_MENU_OPTION("More accurate", SND_EMU2413)
PL_MENU_OPTIONS_END
PL_MENU_OPTIONS_BEGIN(SoundBoostOptions)
  PL_MENU_OPTION("Off", 0)
  PL_MENU_OPTION("2x",  1)
  PL_MENU_OPTION("4x",  2)
PL_MENU_OPTIONS_END
...
PL_MENU_OPTIONS_BEGIN(ScreenSizeOptions)
  PL_MENU_OPTION("Normal (GG 2x)",              DISPLAY_MODE_UNSCALED)
  PL_MENU_OPTION("Integer FS",  DISPLAY_MODE_FIT_HEIGHT)
  PL_MENU_OPTION("Wide (GG Fullbuffer)", DISPLAY_MODE_FILL_SCREEN)
  PL_MENU_OPTION("Aspect Ratio", DISPLAY_MODE_ASPECT)  
  PL_MENU_OPTION("Fullscreen", DISPLAY_MODE_FULLSCREEN)
PL_MENU_OPTIONS_END
...
PL_MENU_OPTIONS_END
PL_MENU_OPTIONS_BEGIN(posxOptions)
  PL_MENU_OPTION("+6", 12)
  PL_MENU_OPTION("+5", 11)
  PL_MENU_OPTION("+4", 10)
  PL_MENU_OPTION("+3", 9)
...
PL_MENU_OPTIONS_END
PL_MENU_OPTIONS_BEGIN(RegionOptions)
  PL_MENU_OPTION("Export",    0)
  PL_MENU_OPTION("Domestic", 1)
PL_MENU_OPTIONS_END
...
PL_MENU_OPTIONS_BEGIN(ButtonMapOptions)
  ...
  PL_MENU_OPTION("Joystick Button I",  JOY|INPUT_BUTTON1)
  PL_MENU_OPTION("Joystick Button II", JOY|INPUT_BUTTON2)
  PL_MENU_OPTION("Joystick Button I (autofire)",  AFI|INPUT_BUTTON1)
  PL_MENU_OPTION("Joystick Button II (autofire)", AFI|INPUT_BUTTON2)
  /* Joystick */
  PL_MENU_OPTION("Start (GG) / Pause (SMS)", SYS|INPUT_START|INPUT_PAUSE)
  PL_MENU_OPTION("Soft Reset (SMS)", SYS|INPUT_RESET)
PL_MENU_OPTIONS_END
...
PL_MENU_ITEMS_BEGIN(OptionMenuDef)
  ...
  PL_MENU_ITEM("Vertical screen position", OPTION_POSX, posxOptions,
               "\026\250\020 Change screen vertical position")               
...
*/
  ...
  PL_MENU_ITEM("Region", OPTION_REGION, RegionOptions,
               "\026\250\020 Change Region") 
  PL_MENU_ITEM("Borders", OPTION_BORDER, ToggleOptions, 
               "\026\250\020 Enable/disable ingame png borders")
...
PL_MENU_ITEMS_BEGIN(SystemMenuDef)
  ...
  PL_MENU_ITEM("FM Emulation", SYSTEM_SOUND_ENGINE, SoundEngineOptions,
    "\026\250\020 Select FM emulation engine")
  PL_MENU_ITEM("Sound Boost", SYSTEM_SOUND_BOOST, SoundBoostOptions,
...
void InitMenu()
{
  ...
  UiMetric.MenuFps = 30;
  UiMetric.TabBgColor = COLOR(0xAD,0x16,0x16,0xff);
  UiMetric.BrowserScreenshotPath = ScreenshotPath;
  UiMetric.BrowserScreenshotDelay = 1;
}
  ...
  do
  {
    ...
    do
    {
      ...
      case TAB_OPTION:
        ...
        item = pl_menu_find_item_by_id(&OptionUiMenu.Menu, OPTION_POSX);
        pl_menu_select_option_by_value(item, (void*)Options.posx);
        ...
        item = pl_menu_find_item_by_id(&OptionUiMenu.Menu, OPTION_REGION);
        pl_menu_select_option_by_value(item, (void*)Options.Region);
        item = pl_menu_find_item_by_id(&OptionUiMenu.Menu, OPTION_BORDER);
        pl_menu_select_option_by_value(item, (void*)Options.Border);          
...
void OnSplashRender(const void *splash, const void *null)
{
  ...
  { 
    ...
    "Version 1.3.2 2014-2015 TheElf",
    "\026http://theelf-megadev.com",
    " ",
  ...
  const char *path, u32 button_mask)
{
  ...
  if (button_mask & PSP_CTRL_LTRIGGER)
  {
    ...
    do
    {
      ...
      if (GAME_LOADED && (TabIndex == TAB_ABOUT)) TabIndex--;       
  ...
  pl_menu_item* item, const pl_menu_option* option)
{
  ...
  else
  {
    ...
    case OPTION_CONTROL_MODE:
      ...
      break;
    case OPTION_REGION:
      Options.Region = (int)option->value;
      break;
    case OPTION_BORDER:
      Options.Border = (int)option->value;
      break;     
    case OPTION_ANIMATE:
...
void LoadOptions()
{
  ...
  Options.posx = pl_ini_get_int(&init, "Video", "Vertical Position", 6);  
  ...
  Options.SoundEngine = pl_ini_get_int(&init, "System", "FM Engine", 1);
  Options.SoundBoost = pl_ini_get_int(&init, "System", "Sound Boost", 0);
  Options.OF = pl_ini_get_int(&init, "System", "OFile", 0);  
  Options.Region = pl_ini_get_int(&init, "Menu", "Region", 0); 
  Options.Border = pl_ini_get_int(&init, "Menu", "Border", 1);      
...
/* Load options */
void LoadOptions_game()
{


  sprintf(IniPath, "%sini/", pl_psp_get_app_directory());
  
  const char *config_name = pl_file_get_filename(CURRENT_GAME);
  pl_file_path path;
  snprintf(path, sizeof(path) - 1, "%s%soptions.ini", IniPath, config_name);
...
static int SaveOptions()
{
  ...
  pl_ini_set_int(&init, "Video", "Vertical Position", Options.posx);  
  ...
  pl_ini_set_int(&init, "Menu", "Region", Options.Region); 
  pl_ini_set_int(&init, "Menu", "Border", Options.Border);    
...
static int CreateOptions()
{
  ...
  pl_ini_set_int(&init, "System", "FM Engine", Options.SoundEngine);
  pl_ini_set_int(&init, "System", "Sound Boost", Options.SoundBoost);
  pl_ini_set_int(&init, "System", "OFile", Options.OF);  
  pl_ini_set_int(&init, "Menu", "Region", Options.Region); 
  pl_ini_set_int(&init, "Menu", "Border", Options.Border);   
  pl_ini_set_int(&init, "Input", "Autofire", Options.AutoFire);
  pl_ini_set_int(&init, "Enhancements", "Rewind Save Rate", Options.RewindSaveRate);
...
static void loadborder()
{
   sprintf(BorderPath, "%sborders/", pl_psp_get_app_directory()); 
   pl_file_path bpath;
 
   if (Options.Border==1){ 
   const char *border_name = pl_file_get_filename(CURRENT_GAME);
   snprintf(bpath, sizeof(bpath) - 1, "%s%sborder.png", BorderPath, border_name);
   bordername = bpath;
   customborder=1;
   
    FILE *fp;
    fp = fopen(bordername,"r");
   
   if (fp == NULL) {
   
   
   if (Options.DisplayMode==0){
    if (IS_GG) {snprintf(bpath, sizeof(bpath) - 1, "%sbordergg.png", BorderPath);   
   bordername = bpath;
   customborder=0;}
   else {
      if (vcodies==1){
   snprintf(bpath, sizeof(bpath) - 1, "%scodemaster.png", BorderPath);}   
      else  {
   snprintf(bpath, sizeof(bpath) - 1, "%sborder.png", BorderPath);}     
   bordername = bpath;
   customborder=0;}}
   
   if (Options.DisplayMode==1 || Options.DisplayMode==4){ 
   snprintf(bpath, sizeof(bpath) - 1, "%scolor.png", BorderPath);
   bordername = bpath;
   customborder=0;}
   
   if (Options.DisplayMode==2 || Options.DisplayMode==3){ 
   snprintf(bpath, sizeof(bpath) - 1, "%sgeneric.png", BorderPath);
   bordername = bpath;
   customborder=0;}
